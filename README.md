...
QuickLaunch는 외부 서버에서 설정이나 실행 대상을 받아오지 않으며, 원격 명령 실행·자동 업데이트·원격 라이브러리 동기화를 제공하지 않습니다. 따라서 외부 사용자가 인터넷을 통해 QuickLaunch를 직접 조종해 트로이목마나 임의 명령을 실행시키는 구조가 아닙니다.

남는 위험은 QuickLaunch 고유 취약점이라기보다 모든 파일 실행 도구에 공통적인 운영 위험입니다. 예를 들어 사용자가 신뢰하지 않는 `.json` 라이브러리 파일, 설치 파일, 실행 파일, 링크를 직접 가져와 클릭하면 위험할 수 있습니다. 이는 이메일 첨부파일, 메신저 파일, 공유폴더 바로가기, 브라우저 다운로드를 열 때와 같은 종류의 위험입니다.

사내 배포 시에는 관리자 또는 공식 배포 경로에서 만든 라이브러리 JSON과 설치 파일만 사용하세요. QuickLaunch는 사용자가 등록한 로컬 바로가기를 실행하는 런처이며, 신뢰하지 않는 파일을 안전한 파일로 바꾸는 보안 샌드박스는 아닙니다.
