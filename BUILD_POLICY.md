...
- 임의 셸 명령어 실행 타입은 제공하지 않습니다.
- 라이브러리에서 사용자 덱으로 추가된 항목은 유형과 실행 대상을 잠급니다.
- 가져온 JSON에서 지원하지 않는 바로가기 타입은 저장하지 않습니다.
- URL 바로가기는 `http://`, `https://`, `ms-screenclip:`만 허용합니다.
- Windows Store/AppX 앱은 `shell:AppsFolder\...` 형식의 앱 ID만 별도로 허용합니다.
...
- `.github/workflows/release.yml`의 `Release` 워크플로로 Windows 빌드를 자동 생성할 수 있습니다.
- 태그(`v*`) 푸시 또는 수동 실행(`workflow_dispatch`)을 지원합니다.
- 릴리스에는 포터블 `QuickLaunch.exe`와 NSIS 설치 파일을 업로드합니다.
- 배포용 관리자 비밀번호가 필요하면 GitHub Secret `ADMIN_PASSWORD`를 설정합니다. 워크플로는 이 값을 사용해 빌드 중 `src/admin-config.js`를 생성합니다.
...
- 패치 적용 후 남은 임시 `.patch` 파일이나 메일 전송용 압축 산출물은 커밋하지 않습니다.
- 관리자 비밀번호는 라이브러리 편집 실수를 막는 UI 잠금이며, 로컬 사용자를 완전히 차단하는 인증 수단으로 취급하지 않습니다.
